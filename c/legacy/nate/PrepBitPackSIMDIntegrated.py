#!/usr/bin/env python3
# scripts/PrepBitPackSIMDIntegrated.py > src/simdintegratedbitpacking.cpp
...
static void __SIMD_integratedfastpack"""+str(bit)+"""_"""+str(length)+"""(__m128i initOffset, const uint32_t *  __restrict__ _in, __m128i *  __restrict__  out) {
    ...
    if (bit != 32):
      print("    __m128i CurrIn = _mm_load_si128(in);");
      print("    __m128i InReg = _mm_and_si128(_mm_sub_epi32(CurrIn, initOffset), mask);");
      print("    initOffset = CurrIn;");
    else:
      print("    __m128i InReg = _mm_load_si128(in);");
    inwordpointer = 0
    ...
    for k in range(ceil((length * bit) / 32)):
      ...
      for x in range(inwordpointer,32,bit):
        ...
        if(valuecounter + 1 < length):
          ...
          if (bit != 32):
            print("    CurrIn = _mm_load_si128(in);");
            print("    InReg = _mm_and_si128(_mm_sub_epi32(CurrIn, initOffset), mask);");
            print("    initOffset = CurrIn;");
          else:
            print("    InReg = _mm_load_si128(in);");
          print("");
  ...
  static void __SIMD_integratedfastunpack1_32(__m128i initOffset, const  __m128i*  __restrict__ in, uint32_t *  __restrict__  _out) {
    ...
    for (unsigned i = 0; i < 8; ++i) {
        OutReg1 = _mm_add_epi32(_mm_and_si128(  _mm_srli_epi32(InReg1,shift++) , mask), OutReg4);
        _mm_store_si128(out++, OutReg1);
        OutReg2 = _mm_add_epi32(_mm_and_si128(  _mm_srli_epi32(InReg2,shift++) , mask), OutReg1);
        _mm_store_si128(out++, OutReg2);
        OutReg3 = _mm_add_epi32(_mm_and_si128(  _mm_srli_epi32(InReg1,shift++) , mask), OutReg2);
        _mm_store_si128(out++, OutReg3);
        OutReg4 = _mm_add_epi32(_mm_and_si128(  _mm_srli_epi32(InReg2,shift++) , mask), OutReg3);
...
static void __SIMD_integratedfastunpack"""+str(bit)+"""_"""+str(length)+"""(__m128i initOffset, const  __m128i*  __restrict__ in, uint32_t *  __restrict__  _out) {
    ...
    for k in range(ceil((length * bit) / 32)):
      for x in range(inwordpointer,32,bit):
        ...
        if (bit != 32):
          print("    OutReg = _mm_add_epi32(OutReg, initOffset);\n"); 
  ...
  print("""
void simdintegratedunpack(__m128i initOffset, const __m128i *  __restrict__ in, uint32_t *  __restrict__  out, const uint32_t bit) {
    switch(bit) {""")
...
void simdintegratedpackwithoutmask(__m128i initOffset, const uint32_t *  __restrict__ in, __m128i *  __restrict__  out, const uint32_t bit) {
    switch(bit) {""")
  print("        case 0: return;\n")
  for bit in range(1,33):
  ...
  /*assumes that integers fit in the prescribed number of bits*/
void simintegratedpack(__m128i initOffset, const uint32_t *  __restrict__ in, __m128i *  __restrict__  out, const uint32_t bit) {
    switch(bit) {""")
  print("        case 0: return;\n")
  for bit in range(1,33):
